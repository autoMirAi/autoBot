    ...
    async def ask(self, scope: str, prompt: str, *, writable: bool) -> CodexResult:
        ...
        async with self._semaphore:
            try:
                ...
                logger.warning("Discarding stale Codex thread for scope %s", mode_scope)
                self.sessions.delete(mode_scope)
                fresh_command = list(base_command)
                fresh_command.extend(
                    [
                        "--sandbox",
                        sandbox,
                        "--skip-git-repo-check",
                ...
                return await self._execute(mode_scope, fresh_command, None)
